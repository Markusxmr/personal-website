	...
	<div>
		{#if $store.isAuthenticated && $store.auth}
			<button
				...
				class="transition-fill-colors text-dark-800 hover:text-dark-900 dark:hover:text-light-600 pointer-events-auto flex flex cursor-pointer items-center justify-center gap-2 font-semibold opacity-100 transition-colors duration-200 duration-200"
				aria-label="User"
				><svg fill="none" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
					><path
		...
		{:else}
			<li class="nav-item">
				<a
					...
					href={`/${$store.meta.baseUri}/auth/signin`}
					class={$page?.route?.id === `/${$store.meta.baseUri}/auth/signin` ||
					$page?.route?.id === `/${$store.meta.baseUri}/auth/signup`
		...
		{#if show}
			...
			>
				...
				<li class="nav-item pointer mb-2" style="cursor: pointer">
					<a href={`/${$store.meta.baseUri}/profile`}>My profile</a>
				</li>
				<li class="nav-item pointer mb-2" style="cursor: pointer">
